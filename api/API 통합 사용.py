...
def Model_2():
    ...
    dfs = []
    # 모델 설정
    model = joblib.load('모델/new_decision_tree_model.pkl')
    for i in nasdaq_list:
        try :
...
def List_total(a,b):
    total_lsit = list(set(a) & set(b)) # 겸치는 종목 만 전달
    print(common_list)
    len(common_list)
...
def Order(file_path,Symbol,Order_num):
    ...
    if Order_num == 0:
        ...
        for j in Account_data['output1']:
            if j['ovrs_pdno'] == symbol:
                ORD_qty = j['ord_psbl_qty']
        #매도
        data = {
                    "CANO": Account,
                    "ACNT_PRDT_CD": Account_Back,
    ...
    while True:
        ...
        for i in stock_data['output1']:
            if i['ovrs_pdno'] not in not_signed_list:
                if float(i['evlu_pfls_rt']) > 7:
                    Order(file_path,i['ovrs_pdno'],1)
                    print(i,'판매')
                    time.sleep(1)
                elif -5>  float(i['evlu_pfls_rt']):
                    Order(file_path,i['ovrs_pdno'],1)
                    print(i,'판매')
                    time.sleep(2)
        ...
        if '22:20:00' >= str_now >= "06:00:00":
            data_save=Account_Check(file_path)
            now = datetime.now()
            data_save_time = now.strftime('%Y-%m-%d')
            with open(f'시스템/data/{data_save_time}data.json','w') as f:
                json.dump(data_save, f, ensure_ascii=False)
            break



        time.sleep(3)  # 10초에 한 번씩 계좌를 확인
...
def main():
    ...
    'C:/Users/sand9/OneDrive/바탕 화면/project/시스템/api/모의투자.txt'
    ...
    print(buy_list)
    print(len(buy_list))
    data_save_time = now.strftime('%Y-%m-%d')#시스템\data
    with open(f'시스템/data/{data_save_time}_추천_list.txt', 'w+') as file:
    ...
    while True:
        if datetime.today().weekday() in range(0, 5):
            ...
            if '22:29:00' > str_now  > "06:00:00" :
                sleep_time = (22 - int(now.strftime('%H')))*60*60+(29 - int(now.strftime('%M')))*60+(23 - int(now.strftime('%S')))
                print(sleep_time,'초 만큼 대기')
                time.sleep(sleep_time)
                print(str_now)
                thread1 = threading.Thread(target=Check_PER, args=(file_path,))
                thread2 = threading.Thread(target=Recommend_Buy, args=(file_path, buy_list))
                print('자동매매 시작')
                thread1.start()
                thread2.start()
                print('알고리즘 마무리 대기')
            ...
            elif '22:29:00' < str_now or "00:00:00" < str_now < "06:00:00" :
                print(now)
                thread1 = threading.Thread(target=Check_PER, args=(file_path,))
                thread2 = threading.Thread(target=Recommend_Buy, args=(file_path, buy_list))
                print('자동매매 시작')
